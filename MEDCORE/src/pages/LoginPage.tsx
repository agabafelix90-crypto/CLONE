...
import { Lock, Mail, Eye, EyeOff, Shield, Clock, AlertCircle } from "lucide-react";
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      if (error.message.includes("Email not confirmed")) {
        setErrorMsg("Your email is not yet verified. Please check your inbox for the verification link we sent during registration.");
    ...
    <div className="min-h-screen flex items-center justify-center bg-muted/30 px-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-muted-foreground mt-1">Sign in to your MediCore account</p>
        </div>

        <form onSubmit={handleLogin} className="bg-card rounded-2xl shadow-elevated p-8 space-y-5 border border-border/50">
          {errorMsg && (
        ...
        <div className="flex items-center justify-center gap-6 mt-6 text-xs text-muted-foreground">
          ...
          <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> 24/7 Available</span>
        </div>
      </motion.div>
    </div>
  );
