...
// ── Overlay Panel (same pattern as LandingPage) ─────────────────────────────
function OverlayPanel({ open, onClose, children, title }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 500, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, background: '#f8faff', width: '100%', height: '100vh', display: 'flex', flexDirection: 'column', animation: 'slideDown 0.35s cubic-bezier(0.34,1.56,0.64,1)' }}>
        {/* Panel nav */}
        <div style={{ height: 64, background: '#0a1e4a', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 5vw', flexShrink: 0, boxShadow: '0 2px 24px rgba(10,30,74,0.3)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: 9, background: 'rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Icon path={ICONS.heart} size={16} color="#7ec8f7" />
            </div>
            <span style={{ fontFamily: "'Sora',sans-serif", fontWeight: 900, color: '#ffffff', fontSize: 17 }}>
              ClinicPro<span style={{ color: '#7ec8f7' }}>System</span>{' '}
              <span style={{ color: 'rgba(255,255,255,0.45)', fontWeight: 400, fontSize: 14 }}>/ {title}</span>
            </span>
          </div>
          <button onClick={onClose}
            style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8, width: 38, height: 38, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'background 0.2s' }}
...
function SectionHead({ tag, title, sub }) {
  return (
    <div style={{ marginBottom: 48, textAlign: 'center' }}>
      ...
      <h2 style={{ fontSize: 'clamp(1.7rem,3.5vw,2.6rem)', fontWeight: 900, letterSpacing: '-1px', color: '#0a1e4a', margin: '0 0 14px', fontFamily: "'Sora', sans-serif" }}>{title}</h2>
      {sub && <p style={{ color: '#4a5c82', fontSize: 15.5, maxWidth: 560, margin: '0 auto', lineHeight: 1.75 }}>{sub}</p>}
    </div>
  );
}
...
const Pricing = () => {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: '#f8faff', color: '#0a1e4a' }}>
      ...
      {/* ── Overlay: LandingPage ── */}
      <OverlayPanel open={showHome} onClose={() => setShowHome(false)} title="Home">
        <LandingPage />
      </OverlayPanel>
      ...
      {/*
        ══ NAV — identical spec to LandingPage ══
          • position: fixed, always visible
          • solid #0a1e4a, no border-bottom, no box-shadow
          • Logo → opens LandingPage overlay
          • Nav: Home | About Us | Log in | Register Free
      */}
      <header style={{
        position: 'fixed',
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', maxWidth: 1200, margin: '0 auto' }}>
          ...
          <button onClick={() => setShowHome(true)}
            ...
            <span style={{ fontSize: 20, fontWeight: 900, color: '#ffffff', letterSpacing: '-0.5px', fontFamily: "'Sora', sans-serif" }}>
              ClinicPro<span style={{ color: '#7ec8f7' }}>System</span>
            </span>
          </button>
          ...
          <nav style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            {/* Home — opens LandingPage overlay */}
            <button onClick={() => setShowHome(true)}
              style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.78)', fontWeight: 600, fontSize: 14, cursor: 'pointer', padding: '8px 14px', borderRadius: 8, transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: 6 }}
              onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.1)'; e.currentTarget.style.color = '#ffffff'; }}
              onMouseLeave={e => { e.currentTarget.style.background = 'none'; e.currentTarget.style.color = 'rgba(255,255,255,0.78)'; }}>
              <Icon path={ICONS.home} size={15} color="currentColor" /> Home
            </button>

            {/* About Us — opens AboutUs overlay */}
            <button onClick={() => setShowAbout(true)}
              style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.78)', fontWeight: 600, fontSize: 14, cursor: 'pointer', padding: '8px 14px', borderRadius: 8, transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: 6 }}
              onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.1)'; e.currentTarget.style.color = '#ffffff'; }}
              onMouseLeave={e => { e.currentTarget.style.background = 'none'; e.currentTarget.style.color = 'rgba(255,255,255,0.78)'; }}>
              <Icon path={ICONS.users} size={15} color="currentColor" /> About Us
            </button>
            ...
            <button onClick={() => navigate('/clinic-registration')}
              style={{ background: '#ffffff', border: 'none', color: '#0a1e4a', fontWeight: 800, fontSize: 14, cursor: 'pointer', padding: '9px 20px', borderRadius: 9, marginLeft: 4, transition: 'opacity 0.2s', boxShadow: '0 4px 16px rgba(0,0,0,0.22)' }}
      ...
      <section style={{
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <h1 style={{ fontSize: 'clamp(2.2rem,5vw,4rem)', fontWeight: 900, color: '#ffffff', letterSpacing: '-2px', margin: '0 0 16px', fontFamily: "'Sora',sans-serif" }}>
            Simple Pricing That<br /><span style={{ color: '#7ec8f7' }}>Scales with You</span>
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 16, maxWidth: 520, margin: '0 auto 32px', lineHeight: 1.75 }}>
          ...
          <div style={{ display: 'flex', gap: 20, justifyContent: 'center', flexWrap: 'wrap' }}>
            {[
              ...
              <a key={label} href={href}
                style={{ display: 'flex', alignItems: 'center', gap: 7, color: 'rgba(255,255,255,0.6)', fontSize: 13, fontWeight: 600, textDecoration: 'none', transition: 'color 0.2s' }}
      ...
      {/*
        ══ PLAN SELECTOR ══
        Redesigned as two large card-style toggle buttons — bold, clear, visually distinct.
        Replaces the flat underline tab style.
      */}
      <section style={{ background: '#f8faff', padding: 'clamp(36px,6vw,56px) 5vw 0' }}>
        <div style={{ maxWidth: 720, margin: '0 auto', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: 'clamp(1.5rem,3vw,2.2rem)', fontWeight: 900, color: '#0a1e4a', letterSpacing: '-0.8px', marginBottom: 8, fontFamily: "'Sora',sans-serif" }}>
            How do you want to run ClinicPro?
          </h2>
          <p style={{ color: '#4a5c82', fontSize: 15, lineHeight: 1.7, marginBottom: 32 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 0 }}>
            ...
            <button onClick={() => setActiveTab('online')}
              ...
              onMouseEnter={e => { if (activeTab !== 'online') { e.currentTarget.style.borderColor = 'rgba(10,30,74,0.3)'; e.currentTarget.style.boxShadow = '0 6px 28px rgba(10,30,74,0.12)'; } }}
              onMouseLeave={e => { if (activeTab !== 'online') { e.currentTarget.style.borderColor = 'rgba(10,30,74,0.12)'; e.currentTarget.style.boxShadow = '0 2px 16px rgba(10,30,74,0.07)'; } }}>
              {activeTab === 'online' && (
            ...
            <button onClick={() => setActiveTab('offline')}
              ...
              onMouseEnter={e => { if (activeTab !== 'offline') { e.currentTarget.style.borderColor = 'rgba(10,30,74,0.3)'; e.currentTarget.style.boxShadow = '0 6px 28px rgba(10,30,74,0.12)'; } }}
              onMouseLeave={e => { if (activeTab !== 'offline') { e.currentTarget.style.borderColor = 'rgba(10,30,74,0.12)'; e.currentTarget.style.boxShadow = '0 2px 16px rgba(10,30,74,0.07)'; } }}>
              {activeTab === 'offline' && (
                <div style={{ position: 'absolute', top: 14, right: 14, width: 22, height: 22, borderRadius: '50%', background: '#7ec8f7', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Icon path={ICONS.check} size={12} color="#0a1e4a" />
                </div>
      ...
      {/* ══ ONLINE TAB ══ */}
      {activeTab === 'online' && (
        <div style={{ padding: 'clamp(50px,8vw,90px) 5vw' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            ...
            {/* Setup fee banner */}
            <div style={{ background: '#0a1e4a', borderRadius: 20, padding: 'clamp(28px,5vw,48px)', marginBottom: 32, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 24, position: 'relative', overflow: 'hidden' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 'clamp(2rem,5vw,3.5rem)', fontWeight: 900, color: '#ffffff', fontFamily: "'Sora',sans-serif", letterSpacing: '-1.5px', lineHeight: 1 }}>Contact Us</div>
              ...
              <button onClick={() => openWhatsApp("Hi! I'd like to know the setup and training fee for ClinicProSystem.")}
                style={{ padding: '14px 28px', borderRadius: 12, border: 'none', background: '#ffffff', color: '#0a1e4a', fontWeight: 800, fontSize: 15, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, whiteSpace: 'nowrap', boxShadow: '0 4px 20px rgba(0,0,0,0.2)', transition: 'transform 0.2s' }}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 20, marginBottom: 40 }}>
              {[
                ...
                <div key={tier} style={{ background: highlight ? '#0a1e4a' : '#ffffff', borderRadius: 20, padding: '32px 28px', border: `1.5px solid ${highlight ? '#0a1e4a' : 'rgba(10,30,74,0.1)'}`, boxShadow: highlight ? '0 12px 48px rgba(10,30,74,0.25)' : '0 4px 20px rgba(10,30,74,0.07)', position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ fontSize: 'clamp(2rem,4vw,2.8rem)', fontWeight: 900, color: highlight ? '#7ec8f7' : '#0a1e4a', fontFamily: "'Sora',sans-serif", letterSpacing: '-1px', lineHeight: 1 }}>{price}</div>
            ...
            {/* Why Online */}
            <div style={{ background: '#0a1e4a', borderRadius: 20, padding: 'clamp(32px,5vw,48px)' }}>
              ...
              <h3 style={{ fontSize: 'clamp(1.4rem,3vw,2rem)', fontWeight: 900, color: '#ffffff', letterSpacing: '-0.5px', marginBottom: 32, fontFamily: "'Sora',sans-serif" }}>6 Reasons to Choose the Cloud</h3>
            ...
            <div style={{ textAlign: 'center', marginTop: 40 }}>
              <button onClick={() => openWhatsApp("I'm interested in the Online Subscription for ClinicProSystem. Can you tell me more?")}
                style={{ padding: '16px 40px', borderRadius: 12, border: 'none', background: '#0a1e4a', color: '#ffffff', fontWeight: 800, fontSize: 16, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 10, boxShadow: '0 8px 32px rgba(10,30,74,0.25)', transition: 'transform 0.2s, box-shadow 0.2s' }}
          ...
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24, marginBottom: 48 }}>
              {[
                ...
                <div key={label} style={{ background: '#0a1e4a', borderRadius: 20, padding: '36px 32px', boxShadow: '0 12px 48px rgba(10,30,74,0.22)', position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ fontSize: 'clamp(1.8rem,4vw,2.8rem)', fontWeight: 900, color: '#ffffff', fontFamily: "'Sora',sans-serif", letterSpacing: '-1px', lineHeight: 1, marginBottom: 12 }}>{price}</div>
            ...
            <div style={{ textAlign: 'center' }}>
              <button onClick={() => openWhatsApp("I'm interested in Offline Installation for ClinicProSystem. Can you provide more details?")}
                style={{ padding: '16px 40px', borderRadius: 12, border: 'none', background: '#0a1e4a', color: '#ffffff', fontWeight: 800, fontSize: 16, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 10, boxShadow: '0 8px 32px rgba(10,30,74,0.25)', transition: 'transform 0.2s' }}
      ...
      {/* ══ Value Props ══ */}
      <section style={{ padding: 'clamp(50px,8vw,80px) 5vw', background: '#ffffff', borderTop: '1px solid rgba(10,30,74,0.06)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      {/* ══ Final CTA ══ */}
      <section style={{ background: 'linear-gradient(150deg,#071430,#0a1e4a,#1a3d82)', padding: 'clamp(60px,10vw,100px) 5vw', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 640, margin: '0 auto' }}>
          <h2 style={{ fontSize: 'clamp(1.8rem,3.5vw,2.8rem)', fontWeight: 900, color: '#ffffff', letterSpacing: '-1px', margin: '0 0 16px', fontFamily: "'Sora',sans-serif" }}>Ready to Get Started?</h2>
          ...
          <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
            <button onClick={() => openWhatsApp("I'd like to learn more about ClinicProSystem. Can we schedule a consultation?")}
              style={{ padding: '15px 32px', borderRadius: 12, border: 'none', background: '#ffffff', color: '#0a1e4a', fontWeight: 800, fontSize: 15, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 10, boxShadow: '0 8px 40px rgba(0,0,0,0.25)', transition: 'transform 0.2s' }}
              onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-3px)'}
              onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}>
              <Icon path={ICONS.whatsapp} size={18} color="#0a1e4a" /> Contact Sales
            </button>
            <button onClick={() => navigate('/clinic-registration')}
              style={{ padding: '15px 32px', borderRadius: 12, border: '1.5px solid rgba(255,255,255,0.3)', background: 'rgba(255,255,255,0.08)', color: '#ffffff', fontWeight: 700, fontSize: 15, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 10, transition: 'all 0.2s' }}
      ...
      {/* ══ Footer ══ */}
      <footer style={{ background: '#06121f', padding: 'clamp(40px,6vw,60px) 5vw 28px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px,1fr))', gap: 40, paddingBottom: 40, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <div>
              <button onClick={() => setShowHome(true)}
              ...
              <p style={{ fontSize: 13.5, color: '#445577', lineHeight: 1.8 }}>Modern AI-powered clinic management. Built for Uganda.</p>
            </div>
            <div>
              ...
              {[
                ['Home',          () => setShowHome(true)],
                ['About Us',      () => setShowAbout(true)],
                ['Register Free', () => navigate('/clinic-registration')],
                ['Log in',        () => navigate('/login')],
              ].map(([l, fn]) => (
                <button key={l} onClick={fn}
                  style={{ display: 'block', fontSize: 14, color: '#445577', background: 'none', border: 'none', cursor: 'pointer', padding: '0 0 10px', textAlign: 'left', transition: 'color 0.2s' }}
            ...
            <div>
              ...
              <button onClick={() => openWhatsApp("Hi, I'd like to chat about ClinicProSystem.")}
                style={{ marginTop: 16, padding: '10px 20px', borderRadius: 9, border: 'none', background: '#0a1e4a', color: '#ffffff', fontWeight: 800, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7, transition: 'background 0.2s', boxShadow: '0 2px 12px rgba(10,30,74,0.4)' }}
